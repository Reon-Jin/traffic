...
# checkpoint = torch.load(checkpointname)  # 替换成实际路径
...
for epoch in range(num_epochs):
    ...
    with torch.no_grad():
        ...
        for batch in test_loader:
            ...
            # 关键修改：取最后一时间步特征 (B,D)
            selected_time_feature = target_time_feature[:, -1, :]  # 切片操作

            # 时间筛选（使用selected_time_feature）
            hour_onehot = selected_time_feature[:, :24]  # 提取小时编码
            hours = torch.argmax(hour_onehot, dim=1)     # 转换为整数小时数

            # 创建时间掩码（示例：早高峰6-9点）
            # 定义复合时间条件
            time_mask = (
                ((hours >= 7) & (hours <= 9))    # 6:00-8:59 AM
                |                              # 逻辑"或"连接
                ((hours >= 17) & (hours <= 20))  # 16:00-19:59 PM
            )
            
            # 应用筛选
            filtered_pred = c_pred_mapped[time_mask]
            filtered_true = y_c_mapped[time_mask]
            
            # 收集结果
            if filtered_pred.shape[0] > 0:  # 避免空张量
                all_c_preds.append(filtered_pred)
                all_y_c.append(filtered_true)
